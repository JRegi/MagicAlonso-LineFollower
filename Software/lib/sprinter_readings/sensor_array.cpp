...
SensorArray::SensorArray(uint8_t numSensors, uint8_t pinS0, uint8_t pinS1, uint8_t pinS2, uint8_t pinS3, uint8_t pinSig, uint8_t ledPin, Color color)
...
void SensorArray::ReadSensors()
{
    for (uint8_t sensorIndex = 0; sensorIndex < _numSensors; sensorIndex++)
    {
        uint16_t lecture = ReadMultiplexer(sensorIndex);
        if (_color == BLACK)
        {
